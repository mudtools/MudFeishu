...
- **服务台 (Helpdesk) 模块**: 新增飞书服务台完整功能支持
  - 新增客服管理相关接口和数据模型
  - 新增客服技能相关 API 和数据模型
  - 新增客服日程相关接口和数据模型
  - 新增工单详情查询接口及相关数据模型
  - 新增工单列表查询、工单更新相关接口与数据模型
  - 新增工单消息相关接口与数据模型
  - 新增工单自定义字段相关 API 及数据模型
- **视频会议纪要**: 新增视频会议纪要相关接口和数据模型
- **事件回调**: 新增多种飞书回调事件类型与处理类
  - 新增邮件接收（Mail Received）事件支持
  - 新增会议室预定配置更新（Reserve Config Updated）事件支持
  - 新增视频会议室相关事件类型与数据模型
  - 新增会议室层级相关回调事件
  - 新增多个会议相关回调事件（会议事件、会议录制开始等）
  - 新增视频会议相关事件类型与数据模型

### 🔧 Changed

- **服务台重构**: 重构客服相关数据模型目录结构，新增日程接口
  - 重构客服日程相关数据模型与接口
- **令牌类型标准化**: 统一替换飞书接口的 Token 参数为枚举类型，新增令牌常量
  - 标准化令牌类型使用方式，并补充 AI 接口文档

### 🐛 Fixed

- 修复 WebSocket 异常断开时重连状态未正确重置的问题

### 📦 Build & Config

- 完善 WebSocket 重连策略与服务优化
- 更新 Mud.HttpUtils 相关包到 1.7.2 版本
- 更新项目版本号至 2.1.6
