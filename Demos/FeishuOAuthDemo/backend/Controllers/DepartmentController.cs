...
public class DepartmentController(IFeishuTenantV3Departments departmentApi) : ControllerBase
{
    private readonly IFeishuTenantV3Departments _departmentApi = departmentApi;
    ...
    /// <returns></returns>
    [HttpPost]
    public async Task<IActionResult> CreateDepartment([FromBody] DepartmentCreateRequest departmentCreateRequest)
    {
        try
        {
    ...
    /// <returns></returns>
    [HttpGet("batch")]
    public async Task<IActionResult> GetDepartmentsByIds([FromQuery][Required] string[] departmentIds)
    {
        try
        {
