...
/// <summary>
/// <para>获取官网职位列表查询参数（查询对象模式，见 AGENTS.md API-2）</para>
/// </summary>
public class WebsiteJobPostListQuery : IQueryParameter
{
    /// <summary>
    ...
    public IEnumerable<KeyValuePair<string, string?>> ToQueryParameters()
    {
        ...
        {
            yield return new KeyValuePair<string, string?>("create_start_time", CreateStartTime);
        }

        if (!string.IsNullOrEmpty(CreateEndTime))
        {
            yield return new KeyValuePair<string, string?>("create_end_time", CreateEndTime);
        }
    }
}
