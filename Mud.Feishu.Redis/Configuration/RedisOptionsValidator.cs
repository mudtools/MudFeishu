...
/// 内部调用 RedisOptions.Validate() 方法，保持验证逻辑一致性。
/// </remarks>
public class RedisOptionsValidator : IValidateOptions<RedisOptions>
{
    ...
    public ValidateOptionsResult Validate(string? name, RedisOptions options)
    {
        if (options == null)
        {
            return ValidateOptionsResult.Fail("RedisOptions 配置不能为 null");
        }
        ...
        {
            return ValidateOptionsResult.Fail($"RedisOptions 配置验证失败: {ex.Message}");
        }
    }
}
