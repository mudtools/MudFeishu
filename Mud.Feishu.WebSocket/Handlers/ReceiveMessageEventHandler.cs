...
/// 接收消息事件处理器
/// </summary>
public class ReceiveMessageEventHandler : DefaultFeishuEventHandler
{
    public ReceiveMessageEventHandler(ILogger<ReceiveMessageEventHandler> logger) : base(logger)
    {
    }
    ...
    public override async Task ProcessBusinessLogicAsync(EventData eventData, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("处理接收消息事件: {Event}", System.Text.Json.JsonSerializer.Serialize(eventData.Event));
