...
builder.Services.AddFeishuWebSocketService(
    feishuOptions => { /* 配置 */ },
    webSocketOptions => {
        webSocketOptions.EnableMultiHandlerMode = true;
        webSocketOptions.ParallelMultiHandlers = true; // 并行执行
    });
```

**多处理器的优势：**
- 🔧 **插件化架构** - 支持插件式功能扩展
- 🎯 **事件监听模式** - 多个组件可同时监听同一事件
- 🔄 **解耦设计** - 降低事件处理器的耦合度
- 🚀 **并行处理** - 多个处理器并行执行，提高性能
...
```csharp
// 注册多个接收消息处理器
services.AddSingleton<ReceiveMessageHandler>();      // 业务处理
services.AddSingleton<MessageAnalyticsHandler>();   // 分析统计
services.AddSingleton<MessageAuditHandler>();       // 审计记录
services.AddSingleton<MessageCacheHandler>();       // 缓存处理

// 所有处理器都会处理 im.message.receive_v1 事件
```

**多处理器最佳实践：**
- 🎯 **功能明确** - 每个处理器有清晰的职责分工
- 🛡️ **异常隔离** - 单个处理器异常不影响其他处理器
- ⚡ **性能考虑** - 避免在处理器中执行耗时操作
...
### v1.2.0
- **多处理器支持** - 支持同一事件类型注册多个处理器
- **并行处理** - 多个处理器可以并行执行，提高性能
- **工厂接口抽象** - 新增 `IFeishuEventHandlerFactory` 接口
- **配置选项增强** - 新增多处理器模式相关配置选项
- **示例处理器** - 提供消息分析和审计处理器示例
- **向后兼容** - 保持与现有单处理器模式的完全兼容

### v1.1.0
- **重构为策略模式**: 将事件处理重构为可扩展的策略模式
- **新增事件处理器接口**: 提供 `IFeishuEventHandler` 接口，支持自定义事件处理器
- **事件处理器工厂**: 实现 `FeishuEventHandlerFactory` 管理和提供事件处理器
- **内置事件处理器**: 为常见事件类型提供内置处理器实现
- **动态处理器注册**: 支持运行时动态注册和注销事件处理器
- **更好的可扩展性**: 用户可以轻松添加新的事件处理器，无需修改核心代码
