...
/// <summary>
/// RateLimitOptions 配置验证器
/// </summary>
/// <remarks>
/// 实现 IValidateOptions 接口，在依赖注入时自动验证配置。
/// 内部调用 RateLimitOptions.Validate() 方法，保持验证逻辑一致性。
/// </remarks>
public class RateLimitOptionsValidator : IValidateOptions<RateLimitOptions>
{
    ...
    public ValidateOptionsResult Validate(string? name, RateLimitOptions options)
    {
        if (options == null)
        {
            return ValidateOptionsResult.Fail("RateLimitOptions 配置不能为 null");
        }
        ...
        {
            return ValidateOptionsResult.Fail($"RateLimitOptions 配置验证失败: {ex.Message}");
        }
    }
}
