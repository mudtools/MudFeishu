...
Base handler classes (such as `DepartmentCreatedEventHandler`) are generated by the `[GenerateEventHandler]` source generator into the **`Mud.Feishu.EventCallback`** namespace, providing type safety and automatic deduplication.

> ⚠️ **Note**: The `Mud.Feishu.Webhook` package does not depend on `Mud.Feishu.EventCallback` — you need to add a reference to the `Mud.Feishu.EventCallback` package yourself before using the base handler classes.
...
using Mud.Feishu.EventCallback;                 // Namespace of the source-generated base classes
...
public class DemoDepartmentEventHandler : DepartmentCreatedEventHandler
{
    ...
    public DemoDepartmentEventHandler(
        ...
        : base(businessDeduplicator, logger)  // Third parameter is an optional IAppKeyAccessor
    {
    ...
    {
        // Event entity fields are located on eventEntity.Object
        ...
        _logger.LogInformation("Initializing department permissions: {DepartmentName}", eventEntity.Object.Name);

        // Notify department leader
        if (!string.IsNullOrWhiteSpace(eventEntity.Object.LeaderUserId))
        {
            _logger.LogInformation("Notifying department leader: {LeaderUserId}", eventEntity.Object.LeaderUserId);
        }
    }
}
```

> 💡 **Tip**: The `HandleAsync` method of the base class is `sealed` (it contains the built-in deduplication logic) and **cannot be overridden**; when extending, you only need to override `ProcessBusinessLogicAsync`.

### Available Base Event Handlers

- `DepartmentCreatedEventHandler` - Department created event
- `DepartmentUpdateEventHandler` - Department updated event
- `DepartmentDeleteEventHandler` - Department deleted event
- More handlers available in the `Mud.Feishu.EventCallback` namespace
...
| `RateLimit.TooManyRequestsMessage`    | string            | "请求过于频繁，请稍后再试" (Chinese default: "Too many requests, please try again later") | Message when rate limit exceeded (customizable) |
...
4. Configure request URL: `https://your-domain.com/feishu/{your-appKey}` (e.g. `https://your-domain.com/feishu/myapp`; the last segment of the URL is resolved as the AppKey to match the corresponding multi-app configuration — replace it with your actual AppKey instead of a fixed "Webhook")
...
Health check thresholds are built into `FeishuWebhookHealthCheck` and are **not configurable** via `appsettings.json`:

- **Degraded**: concurrent-slot utilization ≥ 80%
- **Degraded**: failed-event backlog ≥ 100 (in-memory failed-event store)
- **Unhealthy**: failed-event backlog ≥ 500

The health status data includes details such as available concurrency slots and the failed-event backlog count.
...
```csharp
using Mud.Feishu.EventCallback;   // Namespace of the source-generated base classes (requires the Mud.Feishu.EventCallback package)
...
public class MyDepartmentHandler : DepartmentCreatedEventHandler
{
    ...
    // Only need to implement business logic; HandleAsync of the base class is sealed and cannot be overridden
    protected override async Task ProcessBusinessLogicAsync(
        EventData eventData,
        DepartmentCreatedResult? eventEntity,
        FeishuEventHeader? header,
        CancellationToken cancellationToken = default)
    {
        // eventEntity is already a strongly-typed entity object; event fields are located on Object
        _logger.LogInformation("Processing department: {Name}", eventEntity?.Object?.Name);
    }
}
```
...
Configuration validation happens in two stages to catch issues early:

- **`Build()` stage**: only validates handler registration — at least one handler must be registered and there must be no duplicate handler/interceptor registrations; violations throw `InvalidOperationException`.
- **Options resolution stage**: the option content (tokens, encrypt keys, per-app settings, etc.) is validated by `Validate()` triggered via `PostConfigure` when `FeishuWebhookOptions` is first resolved (typically when the first request is handled or `IOptions<FeishuWebhookOptions>` is first resolved); invalid configuration also throws `InvalidOperationException`.

```csharp
// Build() only validates handler registration
builder.Services.CreateFeishuWebhookServiceBuilder(builder.Configuration)
    .AddHandler<MessageEventHandler>()
    .Build();

// Configuration content is validated when Options are first resolved (PostConfigure -> Validate())
...
| ----------------------------- | ------------------ | --------------------------------------------------------- |
| `GlobalRoutePrefix`           | `"feishu"`         | Global webhook route prefix (the last segment of the URL is the AppKey) |
