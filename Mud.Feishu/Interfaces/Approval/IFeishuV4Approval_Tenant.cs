...
public interface IFeishuTenantV4Approval : IFeishuAppContextSwitcher
{
    ...
    /// <param name="with_option">是否返回外部数据源和假勤控件选项，true 表示返回，false 表示不返回。</param>
    /// <param name="user_id">用户 ID，ID 类型由 user_id_type 参数指定。以用户身份调用时传当前用户 ID，用于数据权限校验。示例值："f7cb567e"</param>
    /// <param name="nested_mutable_group">是否返回完整的多维表格控件，true 表示返回，false 表示不返回。</param>
    /// <param name="cancellationToken"><see cref="CancellationToken"/>取消操作令牌对象。</param>
    ...
    /// <param name="end_time">结束时间，Unix 时间戳（毫秒）。与 start_time 参数构成查询条件。示例值："1547654251506"</param>
