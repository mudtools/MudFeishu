...
public interface IFeishuV4CalendarEvent : IFeishuAppContextSwitcher
{
    ...
    /// <summary>
    /// 分页获取日程参与人列表
    /// <para>以当前身份（应用或用户）获取日程的参与人列表。</para>
    /// <para><see href="https://open.feishu.cn/document/server-docs/calendar-v4/calendar-event-attendee/list-2">接口文档</see></para>
    /// </summary> 
    /// <param name="calendar_id">
    /// <para>日历 ID。</para>
    /// <para>创建共享日历时会返回日历 ID。也可以调用以下接口获取某一日历的 ID。</para>
    ...
    Task<FeishuApiPageListResult<CalendarEventAttendeeInfoResult>?> GetCalendarEventAttendeePageListAsync(
       [Path] string calendar_id,
       [Path] string event_id,
       [Query] bool? need_resource_customization = null,
       [Query] string? page_token = null,
       [Query] int? page_size = Consts.PageSize_20,
       [Query] string? user_id_type = Consts.User_Id_Type,
       CancellationToken cancellationToken = default);
    ...
    /// <summary>
    /// 分页获取日程参与群成员列表
    /// <para>以当前身份（应用或用户）获取日程的群组类型参与人的群成员列表。</para>
    /// <para><see href="https://open.feishu.cn/document/server-docs/calendar-v4/calendar-event-attendee/list">接口文档</see></para>
    /// </summary> 
    /// <param name="calendar_id">
    /// <para>日历 ID。</para>
    /// <para>创建共享日历时会返回日历 ID。也可以调用以下接口获取某一日历的 ID。</para>
