...
public class EventDeduplicationOptionsTests
{
    ...
    /// <summary>
    /// R5/X6 守卫：EventDeduplicationOptions 标注 [Obsolete] 后不得被悄悄移除。
    /// </summary>
    [Fact]
    public void Type_ShouldKeepObsoleteAttribute_R5X6Guard()
    {
        typeof(EventDeduplicationOptions).GetCustomAttribute<ObsoleteAttribute>()
            .Should().NotBeNull(
                "EventDeduplicationOptions 属 R5/X6 判定「双读回落基座」的类型——" +
