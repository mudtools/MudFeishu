# IFeishuTenantV1AIDocument - 租户AI文档识别API

## 功能描述
飞书AI文档接口包括智能文档处理（支持17种证件识别），如：简历信息解析、机动车发票识别、健康证识别、中国护照识别等能力。支持租户管理员通过租户访问令牌进行各类证件与文档的智能识别。
...
**代码示例**
```csharp
var aiDocApi = feishuApp.GetApi<IFeishuTenantV1AIDocument>();
...
```json
{
  ...
  "msg": "success",
  "data": {
    "license_number": "JY12345678901234",
    "company_name": "某某餐饮有限公司"
  }
}
```
...
```json
{
  ...
  "data": {
    ...
    "credit_code": "91110108MA01ABCDEF",
    "legal_person": "李四"
  }
}
```
...
- 文件大小需要小于30M
- 与普通证件识别不同，需使用 `ContractFileUploadRequest` 并指定待抽取字段
