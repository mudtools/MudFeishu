# 飞书 Aily 智能体 - 租户令牌（FeishuTenantV1AilyAgent）

## 接口名称

**飞书 Aily 智能体（租户令牌）** -（`IFeishuTenantV1AilyAgent`）

## 功能描述

提供以租户身份调用飞书 Aily 智能体的能力。飞书 Aily SDK 是一组服务端 OpenAPI 的封装，用于让用户以编程方式调用飞书 Aily（智能伙伴/智能体）的智能体对话、会话管理、附件与产物能力，把它集成到自己的业务系统里。支持上传附件、查询会话列表、获取指定会话信息、创建会话、发起智能体对话（含 SSE 流式输出）、获取对话结果、下载智能体产物和删除会话等操作。
...
| ----------------------------------- | ---------------------- | -------- | --------- |
| CreateAgentAttachmentAsync          | 上传附件               | 租户令牌 | POST      |
| GetAgentChatSessionPageListAsync    | 查询会话列表           | 租户令牌 | GET       |
| GetAgentChatSessionAsync            | 获取指定会话信息       | 租户令牌 | GET       |
| DeleteAgentChatSessionAsync         | 删除会话               | 租户令牌 | DELETE    |
| CreateAgentChatSessionAsync         | 创建会话               | 租户令牌 | POST      |
| CreateAgentChatAsync                | 发起智能体对话         | 租户令牌 | POST      |
| CreateAgentChatStreamAsync          | 发起智能体对话（SSE 流式输出） | 租户令牌 | POST      |
| GetAgentChatAsync                   | 获取对话结果           | 租户令牌 | GET       |
| GetAgentArtifactAsync               | 下载智能体产物         | 租户令牌 | GET       |
...
**说明**：请求体按 `type` 区分附件来源：`type=image/file` 时 `file` 必传（png/jpg/pdf，文件最大 40M、图片最大 5M），此时 `doc_url` 不生效；`type=feishu_doc/bitable` 时 `doc_url` 必传，此时 `file` 不生效。上传返回的附件 ID 可在发起智能体对话时通过 `agent_attachment_ids` 引用。
...
**说明**：下一页标记取返回结果的 `Data.NextPageToken`（同时会序列化为 `page_token`）；`has_more` 为 false 时表示已到末页。
...
**说明**：删除成功时 `Data` 为空对象；删除后该会话下的历史对话不可恢复。
...
**说明**：创建成功后返回会话 ID 与会话名，可在后续发起对话时通过 `SessionId` 复用该会话进行多轮对话。
