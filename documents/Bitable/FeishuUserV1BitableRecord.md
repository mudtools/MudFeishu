...
提供以用户身份管理飞书多维表格记录的能力。记录 record 是多维表格的数据表中的每一行数据都是一条记录（record）。每条记录都有唯一标识 record_id，record_id 在一个多维表格中唯一，在全局不一定唯一。支持新增、更新、查询、删除记录，以及批量操作等。
...
| --------------------------- | ------------ | -------- | --------- |
| AddRecordAsync              | 新增记录     | 用户令牌 | POST      |
| UpdateRecordAsync           | 更新记录     | 用户令牌 | PUT       |
| QueryRecordsPageListAsync   | 查询记录     | 用户令牌 | POST      |
| DeleteRecordAsync           | 删除记录     | 用户令牌 | DELETE    |
| AddRecordsAsync             | 新增多条记录 | 用户令牌 | POST      |
| UpdateRecordsAsync          | 更新多条记录 | 用户令牌 | POST      |
| GetRecordsAsync             | 批量获取记录 | 用户令牌 | POST      |
| DeleteRecordsAsync          | 批量删除记录 | 用户令牌 | POST      |
...
**说明**：新增记录时需传入字段值。通过 `client_token` 可实现幂等操作，避免重复创建。以用户身份操作时，操作记录将关联到当前用户。
