...
## 接口名称
**云文档权限（用户令牌）** -（`IFeishuUserV1DrivePermissions`）

## 功能描述
提供以用户身份管理飞书云文档权限的能力。权限是指在云文档相关资源中，应用或用户对各类云文档资源，如文件夹、文档、电子表格、多维表格、知识库等的可阅读、可编辑、可管理等权限。支持增加/批量增加/更新/移除协作者权限、获取协作者列表、转移所有者、判断用户令牌、更新/获取权限设置、启用/刷新/停用文档密码等操作。适用于需要以用户身份管理云文档权限的业务场景，如用户个人文档权限管理、协作者邀请等。
...
|---------|---------|---------|----------|
| CreatePermissionMemberAsync | 增加协作者权限 | 用户令牌 | POST |
| BatchCreatePermissionMemberAsync | 批量增加协作者权限 | 用户令牌 | POST |
| UpdatePermissionMemberAsync | 更新协作者权限 | 用户令牌 | PUT |
| GetPermissionMemberAsync | 获取云文档协作者 | 用户令牌 | GET |
| DeletePermissionMemberAsync | 移除云文档协作者权限 | 用户令牌 | DELETE |
| TransferOwnerPermissionMemberAsync | 转移云文档所有者 | 用户令牌 | POST |
| GetAuthPermissionMemberAsync | 判断用户云文档权限 | 用户令牌 | GET |
| UpdatePermissionPublicAsync | 更新云文档权限设置 | 用户令牌 | PATCH |
| GetPermissionPublicAsync | 获取云文档权限设置 | 用户令牌 | GET |
| CreatePermissionPublicPasswordAsync | 启用云文档密码 | 用户令牌 | POST |
| UpdatePermissionPublicPasswordAsync | 刷新云文档密码 | 用户令牌 | PUT |
| DeletePermissionPublicPasswordAsync | 停用云文档密码 | 用户令牌 | DELETE |
...
| `need_notification` | `bool?` | ⚪ | 添加权限后是否通知对方，默认值：`false` |
...
**说明**：为指定云文档添加协作者，协作者可以是用户、群组、部门、用户组等。使用用户令牌调用时，`need_notification` 参数有效。
