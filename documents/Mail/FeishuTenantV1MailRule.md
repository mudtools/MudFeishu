...
## 功能描述
飞书邮箱收信规则API接口实现了修改、查询、删除等邮箱收信规则管理功能。支持租户管理员通过租户访问令牌管理企业内所有用户的邮箱收信规则。
...
| :--- | :--- | :--- | :--- |
| CreateUserMailboxRuleAsync | 创建收信规则 | TenantAccessToken | POST |
| DeleteUserMailboxRuleAsync | 删除收信规则 | TenantAccessToken | DELETE |
| UpdateUserMailboxRuleAsync | 更新收信规则 | TenantAccessToken | PUT |
| GetMailboxRuleListAsync | 列出收信规则 | TenantAccessToken | GET |
| ReorderUserMailboxRuleAsync | 对收信规则进行排序 | TenantAccessToken | POST |
...
- 可设置条件（发件人、主题、收件人等）和操作（移动到文件夹、标记、删除等）
...
```csharp
var ruleApi = feishuApp.GetApi<IFeishuTenantV1MailRule>();
var result = await ruleApi.DeleteUserMailboxRuleAsync("user@example.com", "123123123");
...
```csharp
var ruleApi = feishuApp.GetApi<IFeishuTenantV1MailRule>();
var result = await ruleApi.GetMailboxRuleListAsync("user@example.com");
