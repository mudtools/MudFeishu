...
| `WebSocket URL使用不安全的ws://协议` | 抛 `ArgumentException` | 生产必须 `wss://`；仅测试环境设 `AllowInsecureWebSocket = true` |
...
| `SSL证书验证失败` | `SSL证书验证失败（链错误，非自签名根…）` | 证书过期/链不完整；自签名需 `AllowSelfSignedCertificates`，名称不匹配需 `AllowCertificateNameMismatch` |
...
| 重连风暴 | `等待 {Delay}ms 后进行第 N 次重连尝试` | 冷却期（`ReconnectCooldownTime`）+ 指数退避（`ReconnectDelayMs`/`MaxReconnectDelayMs`）已生效；若仍过密，检查是否多实例同时重连 |
...
| 事件丢失 | 检查是否出现 `消息序号验证失败` / `统一去重检查跳过`（此时回 ACK 200，服务端不再重投） | 若是误判重复：核对 SeqID 去重 `scopeKey` 是否在多实例间互相判重 |
